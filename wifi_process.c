...
void send_commands(char *access_token,char*type_action, char *input, const char *body)
{
  ...
  strcat(str_data, access_token);
  strcat(str_data, times);
  strcat(str_data, type_action);
  strcat(str_data, "\n");
  strcat(str_data, new_sign_algorithm);
  strcat(str_data, "\n\n");
  strcat(str_data, input);
  ...
  sprintf(message, "%s %s HTTP/1.1\r\nhost: %s\r\nclient_id: %s\r\nsign: %s\r\nt: %s\r\nsign_method: HMAC-SHA256\r\naccess_token: %s\r\nContent-Type: application/json\r\nContent-Length: %ld\r\n\r\n%s", type_action,pathsign,Region,Client_ID, sign, times, access_token, strlen(body), body);
  char cloud_rep[response_size] = {'\0'};
  char *status_str = NULL;
  printf("message = %s\n",message);
  call_API(message, cloud_rep);
  status_str = strstr(cloud_rep, "200 OK");
  int t = 0;
  while (status_str == NULL && check_internet == true  && t < 10)
  {
