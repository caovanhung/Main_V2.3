...
func New{{.StructName}}() *{{.StructName}} {
    c := New{{.EmbeddedStructName}}({{.TypeName}})
    c.Format = {{.FormatTypeName}}
    c.Permissions = {{.Permissions}}
    {{if .HasMinValue}}c.SetMinValue({{.MinValue}}){{end}}
    {{if .HasMaxValue}}c.SetMaxValue({{.MaxValue}}){{end}}
    {{if .HasStepValue}}c.SetStepValue({{.StepValue}}){{end}}
    {{if .HasDefaultValue}}c.SetValue({{.DefaultValue}}){{end}}
    {{if .UnitName}}c.Unit = {{.UnitName}}{{end}}
...
type Characteristic struct {
	...
	LocalFilePath      string      // Path to the file (e.g. ~/User/Go/src/github.com/brutella/hap/characteristic/brightness.go)
...
func permissions(char *gen.CharacteristicMetadata) string {
	var perms []string
	for _, perm := range char.Properties {
		switch perm {
		case "read":
		...
		case "cnotify":
			perms = append(perms, "PermissionEvents")
		case "uncnotify":
			// TODO(mah)
			break
		default:
